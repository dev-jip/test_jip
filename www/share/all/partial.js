...
!function(G) {
  ...
  _.tap = _.Tap = function() {
    ...
    return function(arg) {
      var args = arguments.length > 1 ? to_mr(arguments) : arguments[0];
  ...
  _.cb = _.callback = function(f) {
    return __.async.apply(null, map(arguments, function(f) {
      function f2() {
        ...
        return new Promise(function(rs) { f.apply(self, args.concat(rs)); });
      }
      return f2._p_cb = true, f2;
    }));
  };
  ...
  function unpack_promise(v) {
    return is_mr(v) ? thenable_mr(v) &&
      _.go(Promise.all(PA == Promise.all ? v : _.toArray(v)), _.to_mr) : thenable(v) && v;
  }
  ...
  function go_async(self, v, fs, i) {
    return new Promise(function(resolve, reject) {
      ...
      (function go(v) {
        do {
          if (i == l) return need_catch ? reject(v) : resolve(fpro(v));
  ...
  function reduce(body, _async) {
    return function f(d, i, m) {
      ...
      return (iter._p_async || iter._p_cb ? _async : body)(data, iter, keys, memo, arguments.length > 2, 0);
    }
  }
  ...
  function _set(obj, key, vORf) {
    obj[key] = _.is_fn(vORf) ? vORf(obj[key], key, obj) : vORf;
  }
  ...
  _.box = function(key, val) {
    ...
    function make_sel(el) {
      return _.isString(el) ? el : (likearr(el) ? el[0] : el).getAttribute('_sel');
    }
  };
  ...
  function go_strict(lazys, data) {
    return _.go(data, map(_.rest(lazys), function(l) {
      ...
      /*if (lazy._p_lzt_l)*/ return _.is_fn(l) ? _.loop(l) : _.loop(l[0], l[1]);
    }));
  }
  ...
  var immediateFn = typeof setImmediate == 'function' ? setImmediate : function(fn) { setTimeout(fn, 0); };
  ...
  function handle(self, deferred) {
    ...
    immediateFn(function() {
      ...
      if (cb === null) return (self._state == 1 ? resolve : reject)(deferred.promise, self._value);
      var ret;
    ...
    try {
      if (val && (typeof val === 'object' || typeof val === 'function')) {
        if (val instanceof Promise) {
          ...
          self._value = val;
          return finale(self);
        } else if (typeof val.then == 'function') return tryp(_.bind(val.then, val), self);
      }
      self._state = 1;
      self._value = val;
      finale(self);
  ...
  function finale(self) {
    if (self._state == 2 && self._deferreds.length == 0) immediateFn(function() {
      if (!self._handled && typeof console != 'undefined' && console)
        console.warn('Possible Unhandled Promise Rejection:', self._value);
    });

    each(self._deferreds, function(def) { handle(self, def) });
  ...
  function tryp(fn, self) {
    var i = 0;
    try { fn(function (val) { i++ || resolve(self, val); }, err); }
    catch (ex) { err(ex); }
    function err(reason) { i++ || reject(self, reason); }
  }
  ...
  Promise.all = PA; function PA(list) {
    return new Promise(function(resolve, reject) {
      ...
      each(list, function res(v, i) {
        try {
          if (thenable(v)) return v.then.call(v, function(v) { res(v, i); }, reject);
          list2[i] = v;
          if (!--l) resolve(list2);
        } catch (e) { reject(e); }
      });
  ...
  Promise.resolve = function(v) {
    return v && v.constructor === Promise ? v : new Promise(function(rs) { rs(v); });
  };
...
/*
 * TODO
 * V 0. _.set -> 리턴 값 하나로 하기
 * V 1. _.set -> 보조 함수 인자 1개로 줄이기
 * 2. L 다시 만들기
 * V 3. 고차 함수 내에 arguments 객체 안보이게 하기 + 중복 줄이기
 * V 4. Promise 자체 구현
 * V 5. Promise reject 잡도록
 * V 6. _.catch
 * V 7. _.has_error가 제대로 돌아가지 않음, _.has_error 제거
 * 8. _.callback 함수 제대로 동작하는지 다양한 경우의 수 체크 필요
 * */